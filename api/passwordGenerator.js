export default function passwordGenerator(
	...
	if (includesUpperCase) {
		...
		guaranteedCharacters.push(
			getRandomCharacterFromArray(characters.uppercase)
		);
	}
	if (includeLowerCase) {
		allowedCharacters = allowedCharacters.concat(
		...
		guaranteedCharacters.push(
			getRandomCharacterFromArray(characters.lowercase)
		);
	}
	if (includeNumbers) {
		allowedCharacters = allowedCharacters.concat(characters.numbers);
		guaranteedCharacters.push(
			getRandomCharacterFromArray(characters.numbers)
		);
	}
	if (includeSymbols) {
		allowedCharacters = allowedCharacters.concat(characters.symbols);
		guaranteedCharacters.push(
			getRandomCharacterFromArray(characters.symbols)
		);
	}
	if (
	...
	for (
		let i = 0;
		i < characterCount - guaranteedCharacters.length;
		i++
	) {
		password.push(
			allowedCharacters[getRandomCharacter(allowedCharacters.length)]
		);
	}
